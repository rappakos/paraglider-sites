...
async def refresh_flight_list(dhv_site_id:int, last_flight_date):
    ...
    start_date = last_flight_date if last_flight_date else MIN_DATE
    #print(start_date)

    query = {"navpars":{"start":0,"limit":PAGE_SIZE,"sort":[{"field":"FlightDate"}]}}
    decoded_url = f"https://de.dhv-xc.de/api/fli/flights?d0={start_date}&fkcat%5B%5D=1&fkto%5B%5D={dhv_site_id}&{urllib.parse.urlencode(query,quote_via=urllib.parse.quote_plus).replace('%27', '%22').replace('+', '')}"
